...
// evalWithLegacyCompat is a funny sort of expr.Value that also supports the
// naked $VARIABLE syntax that our old parser used to support using
// os.ExpandEnv. HCL doesn't support that syntax itself, but we need to
// continue to support it for backward compatibility.
func evalWithLegacyCompat(expr hcl.Expression, ctx hcl.EvalContext) cty.Value {
